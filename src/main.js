...
export function calcTotalAmount(plays, performances) {
	...
	for (const performance of performances) {
		...
		totalAmount += calcAmount(plays, performance, calc);
	}
	return totalAmount;
}
...
export function renderInvoiceTxt(invoice, plays) {
	...
	for (const performance of invoice.performances) {
		invoiceTxt += `・${plays[performance.playID].name} (観客数:${performance.audience}人、金額:$${calcAmount(plays, performance, calc)})\n`;
	}
...
export function renderInvoiceHtml(invoice, plays) {
	...
	for (const performance of invoice.performances) {
		invoiceHtml += `<li>${plays[performance.playID].name} (観客数:${performance.audience}人、金額:$${calcAmount(plays, performance, calc)})</li><br>`;
	}
